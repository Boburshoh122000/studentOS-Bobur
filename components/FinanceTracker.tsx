...
export default function FinanceTracker({ navigateTo }: NavigationProps) {
  ...
  const [summary, setSummary] = useState({ income: 0, expense: 0, balance: 0, recentTransactions: [] });
  ...
  const handleAddTransaction = async (e: React.FormEvent) => {
      ...
      try {
          ...
          setNewTransaction({ amount: '', type: 'EXPENSE', description: '', date: new Date().toISOString().split('T')[0] });
  ...
  return (
    <div className="flex h-screen bg-background-light dark:bg-background-dark text-text-main dark:text-white font-display overflow-hidden relative">
        ...
        <div className={`flex flex-col ${isSidebarExpanded ? 'items-start px-4' : 'items-center'} gap-8 w-full transition-all duration-300`}>
          ...
          <nav className={`flex flex-col ${isSidebarExpanded ? 'items-stretch' : 'items-center'} space-y-2 w-full`}>
             {[
               ...
               { screen: null, icon: 'payments', label: 'Finance Tracker', active: true },
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-background-dark p-8">
            {isLoading ? (
                        ...
                        <div className="bg-white dark:bg-card-dark p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">${summary.income.toFixed(2)}</h3>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white dark:bg-card-dark p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">${summary.expense.toFixed(2)}</h3>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white dark:bg-card-dark p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">${summary.balance.toFixed(2)}</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-card-dark rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 min-h-[400px]">
                        ...
                        {transactions.length === 0 ? (
                            ...
                            <div className="flex flex-col gap-4">
                                {transactions.map((t) => (
                                    <div key={t.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg group hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className={`size-10 rounded-full flex items-center justify-center ${t.type === 'EXPENSE' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                                                <span className="material-symbols-outlined">{t.type === 'EXPENSE' ? 'shopping_bag' : 'work'}</span>
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-gray-900 dark:text-white">{t.description || 'Untitled Transaction'}</h4>
                                                <p className="text-xs text-text-sub">{new Date(t.date).toLocaleDateString()} &bull; {t.category?.name || 'Uncategorized'}</p>
                                            </div>
                                        </div>
