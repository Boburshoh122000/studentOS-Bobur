...
/* ──── Shared ──── */
const CARD =
  'bg-white dark:bg-[#14161f] rounded-2xl border border-gray-100 dark:border-white/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.04)] dark:shadow-none';
...
const LABEL =
  'text-[10px] font-semibold tracking-[0.1em] uppercase text-gray-400 dark:text-gray-500';
...
export default function Dashboard({ navigateTo }: NavigationProps) {
    ...
    <header className="h-16 px-5 md:px-6 flex items-center justify-between bg-white dark:bg-[#0c0e16] border-b border-gray-100 dark:border-white/[0.06] z-10">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label="Credits"
          className="flex items-center gap-1 px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/15 rounded-lg cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-500/15 transition-colors text-xs"
        >
          <span>💎</span>
          <span className="font-bold text-indigo-600 dark:text-indigo-300 tabular-nums">
            {balance}
          </span>
        </button>
  ...
  return (
    <DashboardLayout
      ...
      headerContent={headerContent}
    >
      {/* Single flat grid — fills viewport, no bottom gap */}
      <div className="p-3 sm:p-4 xl:p-5 bg-[#f6f7fa] dark:bg-[#0a0c14] min-h-full">
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 xl:grid-cols-12 gap-3 md:gap-4">
          {/* ── Stat: Active Applications ── */}
          <div
            onClick={() => navigateTo(Screen.CAREER_TRACKER)}
            className={`${CARD_CLICK} md:col-span-2 xl:col-span-3`}
          >
          ...
          {/* ── Recent Applications (6/12 desktop, 4/8 laptop, full tablet) ── */}
          <div className={`${CARD} overflow-hidden md:col-span-4 lg:col-span-4 xl:col-span-6`}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-white/[0.04]">
              ...
              <button
                onClick={() => navigateTo(Screen.CAREER_TRACKER)}
                className="text-[10px] font-bold text-primary hover:text-primary/70 flex items-center gap-1 cursor-pointer transition-colors"
              >
              ...
              {recentApps.length > 0 ? (
                recentApps.slice(0, 5).map((app: any, i: number) => {
                  ...
                  return (
                    <div
                      key={app.id}
                      className={`group flex items-center justify-between px-5 py-2.5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer ${i < Math.min(recentApps.length, 5) - 1 ? 'border-b border-gray-50 dark:border-white/[0.03]' : ''}`}
                    >
                      ...
                      <span
                        className={`inline-flex items-center gap-1 text-[9px] font-bold px-2 py-1 rounded-full flex-shrink-0 ${b.bg} ${b.text}`}
                      >
                        <span className={`w-1 h-1 rounded-full ${b.dot}`} />
                        {t(b.labelKey)}
                      </span>
          ...
          {/* ── Profile Views (3/12 desktop, 2/8 laptop, full tablet) ── */}
          <div className={`${CARD} overflow-hidden md:col-span-4 lg:col-span-2 xl:col-span-3`}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-white/[0.04]">
              <div className="flex items-center gap-2">
            ...
            <div className="flex border-b border-gray-50 dark:border-white/[0.03]">
              <div className="flex-1 px-5 py-3 border-r border-gray-50 dark:border-white/[0.03]">
                ...
                <p className="text-xl font-extrabold text-gray-900 dark:text-white mt-0.5 tabular-nums">
                  3
                </p>
              </div>
            ...
            <div>
              {MOCK_VIEWERS.map((v, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-3 px-5 py-2.5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer ${i < MOCK_VIEWERS.length - 1 ? 'border-b border-gray-50 dark:border-white/[0.03]' : ''}`}
                >
                  <div
          ...
          {/* ── Quick Actions (3/12 desktop, 2/8 laptop, full tablet) ── */}
          <div className={`${CARD} flex flex-col md:col-span-4 lg:col-span-2 xl:col-span-3`}>
            ...
            <div className="flex flex-col gap-0.5 p-2 flex-1">
              <button
                ...
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors text-left cursor-pointer group"
              >
              ...
              {dashboardData?.telegramConnected ? (
                ...
                <a
                  ...
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#229ED9]/5 transition-colors text-left cursor-pointer group"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-[#229ED9]/50">
          ...
          {/* ── CTA Banner — full width, inside grid ── */}
          {stats.profileCompletion < 100 && (
            <div className="col-span-full bg-gray-900 dark:bg-[#0d0f17] text-white rounded-2xl px-6 py-4 flex items-center justify-between gap-4">
              ...
              <button
                onClick={() => navigateTo(Screen.PROFILE)}
                className="px-5 py-2 bg-white text-gray-900 rounded-xl text-xs font-bold hover:bg-gray-100 active:scale-[0.97] transition-all cursor-pointer whitespace-nowrap"
              >
                {t('Dashboard.update_profile')}
              </button>
            </div>
