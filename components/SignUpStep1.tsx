...
export default function SignUpStep1({ navigateTo: _navigateTo }: NavigationProps) {
  ...
  // Step 1: Validate form → Send OTP → Go to step 2
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send OTP to email (don't create account yet)
      const { error: otpError } = await authApi.sendOtp({ email });
      if (otpError) {
        setError(otpError);
        return;
      }
  ...
  return (
    <AuthLayout
      ...
      footerLinkTo={step === 'register' ? '/signin' : undefined}
    >
      {/* Error Message */}
      {error && (
      ...
      {step === 'register' ? (
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={
                isLoading || !email || !password || !fullName || password !== confirmPassword
              }
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-6">
          {/* Email badge */}
          <div className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-200">
            <svg
          ...
          <div className="flex justify-center gap-3" onPaste={handleOtpPaste}>
            {otpDigits.map((digit, index) => (
              <input
                ...
                className="w-12 h-14 text-center text-xl font-semibold border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-slate-900 placeholder:text-slate-300"
                placeholder="·"
