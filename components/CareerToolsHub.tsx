...
const tools = [
  {
    labelKey: 'CareerTools.cv_builder',
    descKey: 'CareerTools.cv_builder_desc',
  ...
  {
    labelKey: 'CareerTools.ats_checker',
    descKey: 'CareerTools.ats_checker_desc',
...
export default function CareerToolsHub({ navigateTo }: NavigationProps) {
  ...
  function timeAgo(dateStr: string) {
    ...
    if (mins < 1) return t('CareerTools.just_now');
    if (mins < 60) return t('CareerTools.mins_ago', { count: mins });
    const hrs = Math.floor(mins / 60);
    if (hrs < 24) return t('CareerTools.hrs_ago', { count: hrs });
    const days = Math.floor(hrs / 24);
    if (days === 1) return t('Common.yesterday');
    if (days < 30) return t('CareerTools.days_ago', { count: days });
  ...
  function getScoreBadge(score: number) {
    const text = t('CareerTools.ats_score', { score });
    if (score >= 70)
      return { text, cls: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' };
    if (score >= 50)
      return {
        text,
        cls: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
      };
    return { text, cls: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' };
  }
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 px-6 text-center gap-3">
                    ...
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                      {t('CareerTools.no_activity_title')}
                    </p>
                    <p className="text-xs text-gray-400 max-w-xs">
                      {t('CareerTools.no_activity_desc')}
                    </p>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                {t('CareerTools.profile_readiness')}
              </h3>
              ...
              <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-3">
                65% {t('CareerTools.complete')}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-5 leading-relaxed">
                {t('CareerTools.profile_desc')}
              </p>
              <button
                type="button"
              ...
              >
                {t('CareerTools.complete_profile')}
              </button>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-white">
                    {t('CareerTools.scan_existing_cv')}
                  </span>
                </button>
              </div>
            </div>
