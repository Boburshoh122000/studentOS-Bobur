/**
 * SAFE icon migration — single-line patterns ONLY.
 * This script NEVER touches multiline spans, dynamic content, or ternary expressions.
 * It handles exactly two patterns:
 *   1. <span className="material-symbols-outlined ...">iconName</span>
 *   2. <span className={`material-symbols-outlined ...`}>iconName</span>
 * Both must be on a SINGLE LINE with a static icon name (no {}).
 */
const fs = require('fs');
...
const ICON_MAP = {
    ...
    'arrow_forward_ios': 'ChevronRight', 'arrow_back_ios': 'ChevronLeft',
    ...
    'more_horiz': 'MoreHorizontal', 'campaign': 'Megaphone',
    'workspace_premium': 'Crown', 'local_activity': 'Ticket',
    'sentiment_satisfied': 'Smile', 'mood': 'Smile',
    'stacked_bar_chart': 'BarChart3', 'category': 'Shapes',
    'view_list': 'List', 'tune': 'SlidersHorizontal',
};
...
for (const filePath of getAllTsxFiles(COMPONENT_DIR)) {
    ...
    // Process LINE BY LINE to prevent cross-line destruction
    const lines = content.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (!line.includes('material-symbols-outlined')) continue;

        // Pattern: single-line <span className="material-symbols-outlined ...">iconname</span>
        // Must have static icon name (no { or } in the text content)
        const match = line.match(
            /<span\s+className=(?:["']|{`)[^"'`]*material-symbols-outlined[^"'`]*(?:["']|`})\s*>(\w+)<\/span>/
        );
        ...
        if (!lucide) {
            console.warn(`  ⚠️  Unknown: "${iconName}" in ${path.basename(filePath)}:${i + 1}`);
            continue;
        }

        // Extract size from the className
        let size = 20;
        const sizeMatch = line.match(/text-\[(\d+)px\]/);
        if (sizeMatch) size = parseInt(sizeMatch[1]);
        else if (line.includes('text-6xl')) size = 48;
        else if (line.includes('text-5xl')) size = 40;
        else if (line.includes('text-4xl')) size = 36;
        else if (line.includes('text-3xl')) size = 30;
        else if (line.includes('text-2xl')) size = 24;
        else if (line.includes('text-xl')) size = 20;
        else if (line.includes('text-lg')) size = 18;
        else if (line.includes('text-sm')) size = 14;
        else if (line.includes('text-xs')) size = 12;

        // Extract meaningful CSS classes (color, animation)
        ...
        const replacement = `<${lucide} size={${size}}${cls} />`;

        // Replace ONLY the matched span on this specific line
    ...
    if (fileCount > 0) {
        ...
        // Add/merge lucide-react imports
        ...
        filesFixed++;
        console.log(`✅ ${path.basename(filePath)}: ${fileCount} icons`);
    }
}

console.log(`\n═══════════════════════════════`);
console.log(`Total: ${totalFixed} icons in ${filesFixed} files`);
