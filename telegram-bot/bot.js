...
function getUserHabits(uid) {
  if (!habitsStore.has(uid)) habitsStore.set(uid, []);
  return habitsStore.get(uid);
}
function getToday() {
...
const addHabitScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    if (habits.some((h) => h.name.toLowerCase() === name.toLowerCase())) {
      await ctx.reply(t(ctx, 'habit_exists', { name }), {
        parse_mode: 'Markdown',
        ...habitMenuKb(ctx),
      });
      return ctx.scene.leave();
    ...
    await ctx.reply(t(ctx, 'habit_added', { name }), {
      parse_mode: 'Markdown',
      ...habitMenuKb(ctx),
    });
    return ctx.scene.leave();
...
async function showHabitList(ctx) {
  ...
  await ctx.reply(t(ctx, 'habit_list'), {
    parse_mode: 'Markdown',
...
bot.hears([i18n.en.btn_add_habit, i18n.uz.btn_add_habit, i18n.ru.btn_add_habit], (ctx) =>
  ctx.scene.enter('add-habit-wizard')
);
...
bot.action(/toggle_(\d+)/, async (ctx) => {
  ...
  const buttons = habits.map((hb, i) => [
    Markup.button.callback(
      `${hb.lastDone === today ? '✅' : '⬜'} ${hb.name}  (🔥${hb.streak})`,
      `toggle_${i}`
    ),
  ]);
  buttons.push([Markup.button.callback(t(ctx, 'habit_delete_btn'), 'delete_menu')]);
  try {
    ...
    // Telegram may reject edits that do not change the current message.
  }
});
...
bot.action(/del_(\d+)/, async (ctx) => {
  ...
  if (habits.length > 0) {
    ...
    const buttons = habits.map((h, i) => [
      Markup.button.callback(
        `${h.lastDone === today ? '✅' : '⬜'} ${h.name}  (🔥${h.streak})`,
        `toggle_${i}`
      ),
    ]);
    buttons.push([Markup.button.callback(t(ctx, 'habit_delete_btn'), 'delete_menu')]);
